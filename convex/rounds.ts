...
export const getParticipantsForRounds = query({
	...
	handler: async (ctx, args) => {
		...
		for (const round of rounds) {
			...
			const players: Doc<"gameParticipants">[] = [];
			for (const u of roundScores.map((p) => p.userId)) {
				const user = await ctx.db.get(u);
				const player = await ctx.db.query("gameParticipants").withIndex("by_user", q => q.eq("playerId", u)).first();
			...
			result.push({
				...
				scores: roundScores.map((score) => ({
					...
					gameMaster: players.find(u => u.playerId === score.userId)?.gameMaster,
					score: score.score,
				})),
			});
		}
