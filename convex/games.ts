...
export const getFinishedGames = query({
    ...
    handler: async (ctx, { n }) => {
        ...
        const winnerIds = finishedGamesWithParticipation
            ...
            .filter((winnerId): winnerId is NonNullable<typeof winnerId> => winnerId !== undefined);
        const locationIds = finishedGamesWithParticipation.map(({ game }) => game.locationId);
        ...
        return finishedGamesWithParticipation.map(({ game, participation }, index) => ({
...
export const getActiveGames = query({
    ...
    handler: async (ctx) => {
        ...
        const allGames = await Promise.all(gameIds.map((gameId) => ctx.db.get(gameId)));
        ...
        const [allPlayers, allLocations] = await Promise.all([
            Promise.all(
                allParticipations.flat().map((participant) => ctx.db.get(participant.playerId)),
            ),
            Promise.all(activeGames.map((game) => game.locationId).map((id) => ctx.db.get(id))),
        ]);

        const playerMap = new Map<string, Doc<"users">>(
            allPlayers.filter((player) => player !== null).map((player) => [player._id, player]),
        );
...
export const createGame = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const round_names = args.gameType === "9 runder" ? round_names_9 : round_names_7;
        ...
        const totalScores = scores.reduce(
            (acc, playerScores) => {
                for (const [userId, score] of Object.entries(playerScores)) {
                    acc[userId as Id<"users">] = (acc[userId as Id<"users">] || 0) + score;
                }
                return acc;
            },
            {} as Record<Id<"users">, number>,
