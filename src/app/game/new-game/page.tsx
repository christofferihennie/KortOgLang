...
const gameSchema = z.object({
    players: z.array(z.string()).min(1, "Må være minst 2, legg til en til spiller"),
...
export default function NewGame() {
    ...
    const onSubmit = async (values: z.infer<typeof gameSchema>) => {
        try {
            ...
            setError(
                err instanceof Error ? err.message : "Noe gikk galt ved opprettelse av spillet",
            );
        } finally {
            ...
            <Authenticated>
                <Form {...form}>
                    <form onSubmit={form.handleSubmit(onSubmit)} className='space-y-6'>
                        ...
                        <FormField
                            ...
                            render={({ field }) => (
                                <FormItem>
                                    ...
                                    />
                                    <FormDescription>Velg hvor spillet skal foregå</FormDescription>
                                    <FormMessage />
                                </FormItem>
                            )}
                        />
                            ...
                            render={({ field }) => (
                                <FormItem className='flex flex-row items-center justify-between rounded-lg border p-3 shadow-sm'>
                                    <div className='space-y-0.5'>
                                        <FormLabel>Spill mester</FormLabel>
                                        <FormDescription>
                                            Huk av dersom man ønsker at den som oppretter spillet
                                            skriver inn alle sine poeng.
                                        </FormDescription>
                                    </div>
                                    <FormControl>
                                        <Switch
                        ...
                        <Button type='submit' disabled={isSubmitting} className='bg-sky-600'>
                            {isSubmitting ? (
                                <>
