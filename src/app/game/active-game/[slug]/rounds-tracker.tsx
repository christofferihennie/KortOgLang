...
export default function RoundTracker(props: {
    ...
    const game = useQuery(api.games.getGameInfo, { gameId: rounds[0].gameId });
        ...
        <Tabs value={round}>
            <div className='flex justify-between'>
                ...
                <Button
                    type='button'
                    onClick={() => {
                        const nextRound = (Number.parseInt(round, 10) + 1) % (rounds.length + 1);
                        setRound(nextRound === 0 ? "1" : nextRound.toString());
                    }}
                >
                    Neste runde
                </Button>
                                                ...
                                                userColors[
                                                    (
                                                        participant.userInfo
                                                            ?.gameColor as keyof typeof userColors
                                                    ) || "slate"
                                                ]
